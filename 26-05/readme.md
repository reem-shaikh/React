    ...
    return (
        ...
        # flipped child is content, flipped will be attached to card when user clicks on it
            <div className="content">
                ...
                # adding emojis dyanamically 
                # these props (props.emoji) are send by Grid.js 

                # shuffled emojis are generated at Helperfunction.js and exported to Grid.js 
                # emojis array is created at Emoji.js and exported to Grid.js
                <p>{props.emoji}</p>
            </div>
        </div>
    )
...
7. Creating a state to keep a memory of which state is matched, when everything is matched, we have completed the game 
> Grid.js 
```bash
import Card from './Card';
import { useEffect, useState } from 'react';
